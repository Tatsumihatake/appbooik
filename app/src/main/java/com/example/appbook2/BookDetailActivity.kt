    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btnStartReading).setOnClickListener {
            val input = EditText(this).apply { 
                ...
                hint = "Halaman ke (Max 200)" 
            }
            AlertDialog.Builder(this).setTitle("Simulasi Baca").setView(input)
                .setPositiveButton("Simpan") { _, _ ->
                    val page = input.text.toString().toIntOrNull() ?: 0
                    lifecycleScope.launch(Dispatchers.IO) {
                        AppDatabase.getDatabase(this@BookDetailActivity).progressDao().saveProgress(
                            ReadingProgress(bookId, title, cover, 200, page, System.currentTimeMillis())
                        )
                        withContext(Dispatchers.Main) { 
                            Toast.makeText(this@BookDetailActivity, "Tersimpan", Toast.LENGTH_SHORT).show() 
                        }
                    }
                }.show()
        }
        ...
        findViewById<Button>(R.id.btnFavorite).setOnClickListener {
            val notif = NotificationCompat.Builder(this, "FAV_CH")
                ...
                .setContentTitle("Favorit!")
                .setContentText("$title ditambahkan ke Favorit")
